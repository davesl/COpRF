...
%   110 is (see above)
%   111 is categoryC10
%   898 is the LREN pRF 12.5min default (5min CCW, 5min bars, 2.5min contracting rings)
%   899 same as 898 with additional 10 HRF mapping trials at the end
%   default: 1.
...
case {89 90 91 92 93 94 898 899}
...
%%        temp0 = upsamplematrix(temp0,[1 4],2,0,'nearest');
  ...
  case {73 74 75 76 77  78 79  89 90 91 92 93 94  101 102 103 104 105 106 898 899}
    % N/A
  end
end
...
case {78 79}
  ...
  totalframesstandard = (16+4*32+16+4*32+16)*15;
case {89 90 91 92 93 94  101 102 103 104 105 106 898 899}
...
case 898 % The 12min20sec CCW, bars, conring stimuli
   
  % Load stimulus frame order
  NewFrames = fullfile(stimulusdir,'workspace_retinotopyCaltsmash.mat');
  load(NewFrames,'NewFramesLREN')
  
  % Add stimulis frames then concat HRF frames
  frameorder = zeros(2,length(NewFramesLREN));
  
  frameorder(2,1:length(NewFramesLREN))   = NewFramesLREN;
  
  % Find stimuli frames (i.e. non-zero) and add random image indices
  stimframes      = find(frameorder(2,:)>0);
  % Need image indices to range between 1 and 100
  randimages        = fix(rand(1,length(stimframes))*100-0.5)+1;
  frameorder(1,stimframes)   = randimages;
  
  
case 899 % The 12min20sec CCW, bars, conring stimuli with 10HRF repeats
  
  % Calc duration for HRF cycles as integer of TR around 30s
  HRFrep_duration = round(30/TR)*TR;
  % 15 frames per second and 10 repeats
  num_repeats = 10;
  HRF_num_frames = round(HRFrep_duration*15*num_repeats);
  ...
  % Generate HRF frames
  circlemask_ind = size(maskimages,3);
  HRF_frames = zeros(1,HRF_num_frames);
  for i =1:num_repeats
      HRF_frames(round(1+(i-1)*HRFrep_duration*15):round((i-1)*HRFrep_duration*15+TR*15))=circlemask_ind;
  end
  
  % Find number of 'padding' frames to start HRF approx 24+TR/2 seconds 
  % after end of main stimulus (rounded to multiple of TR)
  numTRs = round((length(NewFramesLREN)+8*15)/15/TR);
  HRFstartframe = round((numTRs*TR+TR/2)*15);
  PaddingFrames = zeros(1,(HRFstartframe-length(NewFramesLREN)));
  
  % Add stimulis frames then concat HRF frames
  frameorder = zeros(2,length(NewFramesLREN)+length(PaddingFrames)+length(HRF_frames));
  
  frameorder(2,1:length(NewFramesLREN))   = NewFramesLREN;
  frameorder(2,length(NewFramesLREN)+1:length(NewFramesLREN)+length(PaddingFrames)) = PaddingFrames;
  frameorder(2,length(NewFramesLREN)+length(PaddingFrames)+1:end) = HRF_frames;
...
if iscolor
  ...
  [timeframes,timekeys,digitrecord,trialoffsets] = ptviewmovie(images, ...
    frameorder,framecolor,frameduration,fixationorder,fixationcolor,fixationsize,grayval,[],[], ...
      ...
      triggerkey,specialcon,trialtask,maskimages,specialoverlay,dummy_pause);
else
